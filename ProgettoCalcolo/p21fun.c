...
//Extract number of digits from int
...
//Prints an output string from the error
...
//Allocates memory for a matrix on file descriptor returned
...
//Reads the matrix from the file descriptor
...
//Checks if the matrix is squared (M*M)
...
//Function that allocates a 2D array [matrix] and returns its pointer
...
//Function that builds a matrix via its pointer and a file
...
//Function that creates an array from a matrix
...
//Function that prints the matrix mtx
...
//Function that prints the squared matrix mtx
void matrix_square_print(int ** mtx, int m) {
    matrix_print(mtx, m, m);
}

//Function that allocates a matrix from its fd
...
//Function that extracts blocks from the matrix mtx
